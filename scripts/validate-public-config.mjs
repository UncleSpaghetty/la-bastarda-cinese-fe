...
if (!siteUrl) {
  console.error("[build] VITE_PUBLIC_SITE_URL is required. Set it to the canonical production origin, for example https://game.example.it.");
  process.exit(1);
}
...
if (parsed.username || parsed.password || parsed.search || parsed.hash || parsed.pathname !== "/") {
  console.error("[build] VITE_PUBLIC_SITE_URL must contain only the public origin, without credentials, path, query, or hash.");
  process.exit(1);
}
