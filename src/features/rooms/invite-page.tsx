...
export function InvitePage() {
  ...
  return <section className="form-page"><p className="eyebrow">Invito privato</p><h1>Prendi posto.</h1>
    <form onSubmit={(event) => { event.preventDefault(); mutation.mutate(); }} className="panel form-stack">
      ...
      <fieldset><legend>Come vuoi partecipare?</legend><div className="role-selector">
        <label className={role === "PLAYER" ? "selected" : ""}><input type="radio" name="role" checked={role === "PLAYER"} onChange={() => setRole("PLAYER")} /><span className="role-icon">♠</span><span><strong>Giocatore</strong><small>Prendi posto, ricevi le carte e gioca.</small></span><i>{role === "PLAYER" ? "✓" : ""}</i></label>
        <label className={role === "SPECTATOR" ? "selected" : ""}><input type="radio" name="role" checked={role === "SPECTATOR"} onChange={() => setRole("SPECTATOR")} /><span className="role-icon">◉</span><span><strong>Spettatore</strong><small>Guarda il tavolo senza partecipare.</small></span><i>{role === "SPECTATOR" ? "✓" : ""}</i></label>
      </div></fieldset>
