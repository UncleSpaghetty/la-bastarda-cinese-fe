...
test("uses the product voice and clear table entry points", () => {
  subject();
  expect(
...
test("describes the ten as banishing cards on the table", () => {
  subject();
  expect(screen.getByText("Bandisce tutte le carte sul tavolo.")).toBeInTheDocument();
  expect(screen.queryByText(/bandisce.*giocatore/i)).not.toBeInTheDocument();
});
