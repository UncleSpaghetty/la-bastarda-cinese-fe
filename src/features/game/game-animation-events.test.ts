...
const state = ({
  ...
  sequence = 1,
  eventType = "cards.played",
  eventCards = [card("event")],
}: {
  deck?: number;
  table?: ReturnType<typeof card>[];
  hand?: ReturnType<typeof card>[];
  sequence?: number;
  eventType?: string;
  eventCards?: ReturnType<typeof card>[];
} = {}): MatchState => ({
  match_id: "match",
    ...
    recent_events: [
      {
        ...
        type: eventType,
        created_at: "",
        payload: { actor_id: "me", actor_name: "Me", cards: eventCards, card_count: table.length },
      },
    ],
  },
});
...
describe("normalizeGameAnimations", () => {
  it.each([
    ...
    [1, 1],
    [3, 3],
  ])("maps a draw of %i cards to %i visual cards", (draw, expected) => {
    ...
    const current = state({
      ...
      hand: [
        ...
        card("h2"),
        card("h3"),
        ...Array.from({ length: draw }, (_, index) => card(`new-${index}`)),
      ],
    });
    const result = normalizeGameAnimations({
      previous,
      current,
      event: current.payload.recent_events?.[0],
    });
  ...
  it("animates exactly the number of cards in a multiple play", () => {
    ...
    expect(
      normalizeGameAnimations({
        previous,
        current,
        event: current.payload.recent_events?.[0],
      }).find(({ kind }) => kind === "play")?.count
    ).toBe(3);
  });
  ...
  it("banishes only the table for a ten and creates no player movement", () => {
    ...
    expect(
      result.some(({ from, to }) => from.startsWith("player") || to.startsWith("player"))
    ).toBe(false);
  });
