...
function banishesTable(previous: MatchState, event: PublicEvent) {
  ...
  if (event.type !== "cards.played" || previous.payload.table_cards.length + played.length === 0) return false;
...
export function normalizeGameAnimations({
  ...
  if (event.type === "table.collected") {
    ...
    return count > 0 ? [{
      id: `${sequence}-collect`, kind: "collect", count,
      from: "table", to: actorId ? `player:${actorId}` : "player", actorId, cardIds: [],
    }] : [];
  }
  ...
  if (banishesTable(previous, event) && current.payload.table_cards.length === 0) {
    ...
    if (count > 0) animations.push({
      id: `${sequence}-banish`, kind: "banish", count,
      from: "table", to: "banished", actorId, cardIds: [],
    });
    return animations;
  }

  if (playedCount > 0) animations.push({
    id: `${sequence}-play`, kind: "play", count: playedCount,
    from: actorId ? `player:${actorId}` : "player", to: "table", actorId,
    cardIds: newlyVisibleCardIds(previous.payload.table_cards, current.payload.table_cards).slice(0, playedCount),
  });
  ...
  if (drawCount > 0) {
    ...
    animations.push({
      id: `${sequence}-draw`, kind: "draw", count: drawCount,
      from: "deck", to: actorId ? `player:${actorId}` : "player", actorId,
      cardIds: newlyVisibleCardIds(previousActor?.private_hand ?? [], currentActor?.private_hand ?? []).slice(0, drawCount),
    });
  }
  return animations;
}
