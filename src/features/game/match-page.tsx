...
import { CardMovementLayer, GameArena, GameEventLog, GameHeader, LocalPlayerDock, SharedTable } from "./game-table-components";
import { useGameAnimations } from "./use-game-animations";

const specialCards: Record<string, string[]> = {
  "2": ["Due trasparente: regole azzerate, talento ancora da trovare.", "Un 2 salva tutti. Perfino chi non lo meritava.", "Reset totale. La strategia era sopravvalutata comunque."],
  "7": ["Sette bastardo: adesso si vola basso. Come le aspettative.", "Sotto il sette, proprio dove sta il livello del tavolo.", "Il 7 ha abbassato l'asticella. Finalmente alla vostra portata."],
  "8": ["Otto! Il prossimo salta. Una pausa dalla sua mediocrità.", "Turno saltato. Il tavolo ringrazia.", "L'8 ti ha ignorato con la stessa eleganza del gruppo chat."],
  "10": ["Dieci: tavolo bruciato. Restano solo le vostre pessime scelte.", "Pulizia completa. Per la dignità è troppo tardi.", "Il 10 cancella il tavolo, non gli errori che vi hanno portato qui."],
  A: ["Asso servito. Scegli a chi rovinare la giornata.", "Un asso e improvvisamente tutti evitano il contatto visivo.", "Passa il disastro a un amico. È questo il senso dell'amicizia."],
};
...
export function MatchPage() {
  ...
  const [historyOpen, setHistoryOpen] = useState(() => typeof window !== "undefined" && window.matchMedia("(min-width: 1366px)").matches);
  ...
  useEffect(() => {
    ...
    if (latest.payload.special_message) setSpecialNotice(latest.payload.special_message);
      ...
      if (special) setSpecialNotice(specialCards[special.rank][latest.sequence % specialCards[special.rank].length]);
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => { setSelected([]); queryClient.invalidateQueries({ queryKey: ["match", id] }); },
    onError: (error) => {
  ...
  const layout = useMemo(() => assignPlayerSeats({ players: state.data?.payload.players ?? [], localPlayerId: own?.id ?? "spectator" }), [own?.id, state.data?.payload.players]);
  ...
  const ownForDisplay = own ? { ...own, private_hand: own.private_hand ? sortCards(own.private_hand) : own.private_hand } : undefined;
  return <section className={`game-table-page ${own ? "" : "is-spectator"}`}>
    ...
    <CardMovementLayer animations={animations} />
    ...
    <GameHeader matchId={id} phase={state.data.payload.phase} activeName={active?.display_name} deadline={state.data.deadline} isOwnTurn={isOwnTurn} onBack={() => navigate(-1)} onHistory={() => setHistoryOpen((open) => !open)} historyCount={historyOpen ? 0 : state.data.payload.recent_events?.length ?? 0} onExit={() => navigate("/")} />
    {!own && <div className="spectator-banner" role="status">Modalità spettatore · stato pubblico in sola lettura</div>}
    <div className={`game-stage ${historyOpen ? "with-history" : ""}`}>
      <GameArena layout={layout} turnSeat={state.data.payload.turn_seat}
        table={<SharedTable cards={state.data.payload.table_cards} deckCount={state.data.payload.deck_count} constraint={state.data.payload.constraint} pendingEffect={state.data.payload.pending_effect} />}
        localDock={ownForDisplay ? <LocalPlayerDock player={ownForDisplay} active={isOwnTurn} selected={selected} onToggle={toggle} onCovered={chooseCovered} busy={mutation.isPending} onPlay={() => mutation.mutate({ name: "play_cards", payload: { card_ids: selected } })} onCollect={() => mutation.mutate({ name: "collect_table", payload: {} })} /> : undefined}
      />
      <GameEventLog events={state.data.payload.recent_events} open={historyOpen} onClose={() => setHistoryOpen(false)} />
      {historyOpen && <button className="event-log-backdrop" type="button" aria-label="Chiudi eventi" onClick={() => setHistoryOpen(false)} />}
    </div>
  </section>;
}
