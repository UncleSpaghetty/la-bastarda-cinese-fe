...
export function TurnTimer({
  ...
  const urgency = ratio <= 0.2 ? "critical" : ratio <= 0.45 ? "warning" : "safe";
  return (
    <div
      className={`turn-timer ${urgency} ${compact ? "compact" : ""}`}
      role="timer"
    ...
    >
      ...
      <div className="timer-track">
        <span className="timer-fill" style={{ transform: `scaleX(${ratio})` }} />
      </div>
    </div>
  );
}
